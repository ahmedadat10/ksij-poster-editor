...
function addDaySection(columnSide) {
  ...
  const targetColumn = columnSide === 'left' ? columns[0] : columns[1];
  ...
  newDay.innerHTML = `
    ...
    <div class="day-body">
      ...
      <button class="add-prayer-btn" onclick="addPrayerRow('${dayId}')">+ Add Prayer Row</button>
      <button class="add-notice-btn" onclick="addNoticeBox('${dayId}')">+ Add Notice</button>
      <button class="add-programme-btn" onclick="toggleProgramme('${dayId}')">+ Add Programme</button>
      <div class="programme-section" id="prog-${dayId}">
        ...
        <button class="add-item-btn" onclick="addScheduleItem('${dayId}')">+ Add Schedule Item</button>
  ...
  // Append to column
  targetColumn.appendChild(newDay);
...
function addDeleteDayButtons() {
  ...
  document.querySelectorAll('.day-section').forEach(daySection => {
    ...
    // Add to day-band (the header)
    const dayBand = daySection.querySelector('.day-band');
    if (dayBand && !dayBand.querySelector('.delete-day-btn')) {
      dayBand.appendChild(deleteBtn);
    }
  });
}
...
function setupAddDayButtons() {
  ...
  if (columns[1]) {
    ...
    columns[1].appendChild(rightBtn);
  }
}
