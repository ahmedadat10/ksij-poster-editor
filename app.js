// KSIJ Poster Editor App - ENHANCED THEME HELPER
...
function changeTheme(theme) {
  ...
  if (isBadge || isHeaderBox) {
    console.log('🔍 Looking for parent programme section...');
    const programmeSection = currentContextTarget.closest('.programme-section');
    
    if (programmeSection) {
      console.log('✅ Found programme section:', programmeSection.id);
      
      // Find ALL header boxes and badges within this programme section
      const headerBoxes = programmeSection.querySelectorAll('.prog-header-box');
      const badges = programmeSection.querySelectorAll('.prog-badge');
      
      console.log(`📦 Found ${headerBoxes.length} header boxes and ${badges.length} badges`);
      
      // Change ALL header boxes theme
      headerBoxes.forEach(headerBox => {
        ...
        console.log('✅ Header box updated');
      });
      
      // Change ALL badges theme
      ...
      console.log(`✅ Programme theme changed to ${theme} (all headers and badges)`);
    } else {
      console.log('❌ Programme section not found!');
    }
  }
  ...
  // Hide the context menu after changing theme
  hideContextMenu();
}
